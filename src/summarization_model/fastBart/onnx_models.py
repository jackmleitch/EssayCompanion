...
class BartDecoder(torch.nn.Module):
    def __init__(self, decoder_sess):
        super().__init__()
        self.decoder = decoder_sess
...
class OnnxBart(BartForConditionalGeneration):
    ...
    def __init__(self, model_or_model_path, onnx_model_sessions):
        ...
        self.encoder = BartEncoder(encoder_sess)
        self.decoder = BartDecoder(decoder_sess)
        self.decoder_init = BartDecoderInit(decoder_sess_init)
...
def get_onnx_model(
    ...
    model = OnnxBart(model_name_or_path, model_sessions)
